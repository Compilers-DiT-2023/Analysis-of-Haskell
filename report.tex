...
\label{sec:org6f31a01}

\subsection{History of Haskell}
\label{sec:orgfed65b1}
...
\begin{table}[htbp]
\caption{\label{tab:org5a3702f}Operators and their types in Haskell's prelude \autocite{fagerbergDM22ProgrammingLanguages}}
\centering
\begin{tabular}{l|r}
\hline
...
\end{aligned}
\end{displaymath}
\captionof{figure}{\label{orgf9a7014}The \emph{integer} in this rule is the precedence of the operator.}
\end{figure}


\begin{sidewaystable}[htbp]
\caption{\label{tab:org4ddc812}The operators included in the prelude with their precedence and fixity \autocite{marlowHaskell2010Language2010}.}
\centering
...
\texttt{ghc} uses a ``parallel generational-copying garbage collector'' \autocite{marlowParallelGenerationalcopyingGarbage2008} that traverses the live data of the running program. The two generations are Generation 0 and 1 (figure \ref{fig:org565b1ad}). An illustration of the garbage collection steps of \texttt{ghc} is shown in figure \ref{fig:org9e8fb78}.

\begin{figure}[htbp]
\centering
\includegraphics[width=5cm]{img/2023-05-24_16-21-37_screenshot.png}
\caption{\label{fig:org9e8fb78}A general overview of the steps of garbage collection \autocite{yangHowGrinchStole}}
\end{figure}

\paragraph*{Newly generated data}
\label{sec:orgc4f5eeb}
...
\caption{\label{fig:org565b1ad}Two-generation GC of \texttt{ghc} \autocite{channableLessonsManagingHaskell}}
\end{figure}

\paragraph*{Collecting the garbage}
\label{sec:org73d3bc1}
...
\paragraph*{Copying}
\label{sec:orgf7b3047}

The visited pieces of data are then copied in parallel to a newly allocated heap \autocite{marlowParallelGenerationalcopyingGarbage2008} (figure \ref{fig:org080d470}). This process is called ``evacuation'' \autocite{yangHowGrinchStole}.

\begin{figure}[htbp]
\centering
\includegraphics[width=8cm]{img/2023-05-24_16-16-17_screenshot.png}
\caption{\label{fig:org080d470}The copy mechanism of the \texttt{ghc} GC \autocite{channableLessonsManagingHaskell}}
\end{figure}
...
Specifically from the class declarations we have the following: A class declaration results in the creation of class methods, represented by variables\textsubscript{i}. The class methods defined within the class declaration are not limited to the class declaration itself, but can be accessed and used outside of it. Every class method will have this kind of type, \mintinline{haskell}{method :: context => type}.
