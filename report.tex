...
\label{sec:orgaead605}

\subsection{History of Haskell}
\label{sec:orga801fd6}
...
David Turner created the lazy functional language Miranda \autocite{turnerMirandaNonstrictFunctional1985} in the mid 1980's. This language in particular had a significant impact on the development of Haskell (see \hyperref[sec:org4fb2c67]{Haskell and Miranda}).
\end{itemize}

\paragraph*{Functional programming and lambda calculus}
\label{sec:orgd54938a}
...
\label{sec:org682b756}

1970--1980 was a time period in which \emph{lazy evaluation} (see \ref{sec:orgc009bb4}) was getting more and more attention. It had been independently invented at least three times and there were many tries in implementing programming languages that supported it as a feature \autocite{hudakHistoryHaskellBeing2007}.
...
The first meeting was held at Yale during 1988. There, a clear list of Haskell's goals was defined (\hyperref[sec:orgd060961]{Haskell's goals}) and a name was decided, that was although later changed (\hyperref[sec:org5c368ae]{Haskell's name}).

\subparagraph*{Haskell's name}
\label{sec:org5c368ae}
...
The PYPL index ranks Haskell in the 28th spot, as of May of 2023 \autocite{carbonnellePYPLPopularitYProgramming2023}. Haskell's evolution in popularity from 2005 until today is shown in figure \ref{fig:org4ea7d5b}. The PYPL (PopularitY of Programming Language) index is based on the number of programming language specific tutorials are searched on Google \autocite{carbonnellePYPLPopularitYProgramming2023}.
...
\caption{\label{fig:org4ea7d5b}Haskell's worldwide popularity since 2005 in logarithmic scale \autocite{carbonnellePYPLPopularitYProgramming2023}}
\end{figure}

\subsection{Use cases}
\label{sec:org6b96462}

\subsubsection{Spam Filtering at Facebook}
\label{sec:org374379f}
...
\subsection{Tools and frameworks written in Haskell}
\label{sec:org1792ac3}

\subsubsection{Hackage}
\label{sec:org7acab38}
...
\label{sec:orgea0227e}

Stack is a popular build tool for Haskell that was originally created to solve problems that developers were facing with Cabal (see \ref{sec:orgd582d5b}), another Haskell build tool \autocite{CommercialhaskellStack2023}.
...
\label{sec:orga34295b}

\subsection{Control Structures}
\label{sec:orga3841bb}
...
The syntax for the \texttt{case} clause is shown in listing \ref{lst:org6f3ebee}, and in listing \ref{lst:orgd3f73b1} the \texttt{if-then-else} clause implemented with \texttt{case} is shown.
...
case expression of
    ...
    patternN -> resultN
\end{minted}
\caption{\label{lst:org6f3ebee}\texttt{case} clause}
\end{listing}
...
\end{minted}
\caption{\label{lst:orgd5c6ec7}Example of a function declaration using guards}
\end{listing}

\subsubsection{Monads}
\label{sec:org78bce64}

\paragraph*{Definition}
\label{sec:orgf03a579}
...
\subsection{Keywords}
\label{sec:org121d4fc}

\subsubsection{\texttt{data}}
\label{sec:org3d02b06}
...
\label{sec:org272b290}

This keyword relates to paragraph \hyperref[sec:org09282b8]{\texttt{data family}}. Specifically, they enable the declaration of types. So this keyword offers a guide for the execution of the above types.

\subsubsection{\texttt{default}}
\label{sec:org7a3d047}
...
data BinaryTree a = Node a !(BinaryTree a) !(BinaryTree a)
  | Leaf
\end{minted}
\caption{\label{lst:orgfa78e84}Definition of the \texttt{BinaryTree} datatype.}
\end{listing}
...
Also, the exclamation point is further utilized denoting rigor within patterns (see listing \ref{lst:orgce75861}). The function uses a bang pattern. This happens in order to denote that its arguments \texttt{x} and \texttt{y} must be calculated strictly prior to the application of the function. This ensures that any possible delay in evaluating the arguments is eliminated, which can improve performance in certain scenarios.
...
g !! y = x * y
\end{minted}
\caption{\label{lst:orgce75861}Pattern matching}
\end{listing}

\subsection{Haskell's Prelude}
\label{sec:org3300ed4}
...
\label{sec:orgb45fe26}

\subsubsection{In general}
\label{sec:org378de30}
...
\begin{table}[htbp]
\caption{\label{tab:org27dcb60}Operators and their types in Haskell's prelude \autocite{fagerbergDM22ProgrammingLanguages}}
\centering
\begin{tabular}{l|r}
\hline
...
\end{aligned}
\end{displaymath}
\caption{\label{orgff29eaa}Haskell operators in BNF form}
\end{figure}

\subsubsection{Fixity Declarations}
\label{sec:org6313ff5}
...
For every operator the precedence and the fixity have to be declared. This declaration in BNF is shown in figure \ref{org7199ced}.
...
\end{aligned}
\end{displaymath}
\captionof{figure}{\label{org7199ced}The \emph{integer} in this rule is the precedence of the operator.}
\end{figure}


\begin{sidewaystable}[htbp]
\caption{\label{tab:org70646bc}The operators included in the prelude with their precedence and fixity \autocite{marlowHaskell2010Language2010}.}
\centering
...
> [2, 3, 3]
\end{minted}
\caption[higher-order lambda]{\label{lst:org52228b0}This lambda takes one argument and returns its value incremented by 1. It is then passed to a higher-order function \texttt{map}}
\end{listing}

The code in listing \ref{lst:org52228b0} uses the \texttt{map} function to apply the lambda to each one of the elements as it iterates the list. The resulting list contains the result of each addition.
...
> Just 9
\end{minted}
\caption{\label{lst:org4f9929f}\texttt{>{}>{}=} operator}
\end{listing}
...
main = do
    ...
    putStrLn ("Hello, " ++ name ++ "!")
\end{minted}
\caption{\label{lst:orgf65c707}How monads are able to perform I/O in Haskell}
\end{listing}

In listing \ref{lst:orgf65c707} firstly the \texttt{main} function is defined as an \texttt{IO} computation and the \texttt{putStrLn} function is used to print a message to the console. After that the \texttt{getLine} function is used to read a line of text given from the user as input. The \texttt{name} variable is bound to the result of \texttt{getLine} using the \texttt{<-} operator, which is a shorthand for using \texttt{>{}>{}=}. Finally, there is a second \texttt{putStrLn} call used to print a ``Hello <name>'' to the console.
...
\label{sec:org66abe72}

\texttt{ghc} uses a ``parallel generational-copying garbage collector'' \autocite{marlowParallelGenerationalcopyingGarbage2008} that traverses the live data of the running program. The two generations are Generation 0 and 1 (figure \ref{fig:orgbd39617}). An illustration of the garbage collection steps of \texttt{ghc} is shown in figure \ref{fig:org9ba5901}.

\begin{figure}[htbp]
\centering
\includegraphics[width=5cm]{img/2023-05-24_16-21-37_screenshot.png}
\caption{\label{fig:org9ba5901}A general overview of the steps of garbage collection \autocite{yangHowGrinchStole}}
\end{figure}

\paragraph*{Newly generated data}
\label{sec:orga7eaa1b}
...
\caption{\label{fig:orgbd39617}Two-generation GC of \texttt{ghc} \autocite{channableLessonsManagingHaskell}}
\end{figure}

\paragraph*{Collecting the garbage}
\label{sec:org4ca6767}
...
\paragraph*{Copying}
\label{sec:org9d150aa}

The visited pieces of data are then copied in parallel to a newly allocated heap \autocite{marlowParallelGenerationalcopyingGarbage2008} (figure \ref{fig:orge78bb90}). This process is called ``evacuation'' \autocite{yangHowGrinchStole}.

\begin{figure}[htbp]
\centering
\includegraphics[width=8cm]{img/2023-05-24_16-16-17_screenshot.png}
\caption{\label{fig:orge78bb90}The copy mechanism of the \texttt{ghc} GC \autocite{channableLessonsManagingHaskell}}
\end{figure}
...
\label{sec:org43866f4}

\subsubsection{Definition}
\label{sec:org88e5d89}
...
\end{minted}
\caption{\label{lst:org08ce773}A class definition in C\#}
\end{listing}
...
m    :: (Bool, Int) -> C -> IO Int
\end{minted}
\caption{The equivalent ``class'' in Haskell (listing \ref{lst:org08ce773})}
\end{listing}

\subsubsection{Objects}
\label{sec:orga48b26a}
...
Type classes can be used to abstract over container types, and how class methods can have polymorphic type signatures that may involve additional type variables and constraints beyond the main type parameter of the class (listing \ref{lst:org2878562}).
...
size :: c a -> Int
\end{minted}
\caption{\label{lst:org2878562}Polymorphic type signatures}
\end{listing}
...
Finally, in Haskell a default class method is a method that provides a default implementation. It is offered for a class method in case it is not defined explicitly in an instance declaration. In Haskell, a class declaration can have a \texttt{where} keyword, which contains the method signatures for the class methods. A class declaration may also not have this \texttt{where} keyword. The second case is used in order to merge many classes to one bigger class that inherits the methods of the smaller ones (listing \ref{lst:orgb4328ab}).
...
absSquared :: a -> a
\end{minted}
\caption[Class inheritance]{\label{lst:orgb4328ab}Class inheritance.In this example, the \texttt{OrderedNum} class combines the \texttt{Ord} and \texttt{Num} classes and defines a new method \texttt{absSquared} that is specific to \texttt{OrderedNum} instances.}
\end{listing}
...
\label{sec:org8933003}

\subsection{The ``Hello, World!'' program in Haskell:}
\label{sec:org53aab92}

\begin{listing}[htbp]
\begin{minted}[]{haskell}
main = putStrLn "Hello, World!"
\end{minted}
\caption{\label{lst:orga15e92a}``Hello, World!'' program}
\end{listing}

The program in listing \ref{lst:orga15e92a} defines a main function that uses the \texttt{putStrLn} function to print the string \texttt{Hello, World!} to the console.

To run this program, you can save it in a file with a \texttt{.hs} extension. Then compile and execute it from the command line using the commands shown in listing \ref{lst:org1fcf28e}.
...
$
\end{minted}
\caption{\label{lst:org1fcf28e}How to compile and run a program with the \texttt{ghc} compiler}
\end{listing}
...
\label{sec:org2c550a7}

After you compile a Haskell program there will be 3 new files (listing \ref{lst:org676b64b}).
...
\end{minted}
\caption{\label{lst:org676b64b}Files generated by \texttt{ghc}}
\end{listing}
...
main = do
    ...
    putStrLn ("The factorial of " ++ show n ++
               " is " ++ show (factorial n))
\end{minted}
\caption{\label{lst:orgc73a50e}The program takes the number \texttt{n} as input, calculates and prints \texttt{n}!}
\end{listing}

The program in the listing \ref{lst:orgc73a50e} defines a main function that prompts the user to enter a number, reads the number from the console using the \texttt{readLn} function, computes its factorial using the \texttt{factorial} function, and prints the result to the console using the \texttt{putStrLn} function.
...
\end{minted}
\caption{Executing listing \ref{lst:orgc73a50e}}
\end{listing}

\subsection{Lambda functions}
\label{sec:orgd50f715}
...
\end{minted}
\caption{\label{lst:org388682f}Sum and product of an integer list}
\end{listing}
...
\end{minted}
\caption{Result of listing \ref{lst:org388682f}}
\end{listing}

As you can see in listing \ref{lst:org388682f}, Haskell provides a really easy way to create an new array with integers ranging from \texttt{a} to \texttt{b} (\texttt{b} included). Even high level languages like Python do not provide a similar feature. You can exclude \texttt{b} using the \texttt{..:} operator. These lambdas were implemented for demonstration purposes, Haskell already provides lambdas like \texttt{sum}, \texttt{product}, \texttt{maximum} and \texttt{minimum}.
...
\label{sec:orgc0b21ec}

To start a \texttt{GHCi} session use the command \texttt{ghci} (listing \ref{lst:orge9f13f3}).
...
ghci>
\end{minted}
\caption{\label{lst:orge9f13f3}Using the \texttt{ghci} interpreter.}
\end{listing}
