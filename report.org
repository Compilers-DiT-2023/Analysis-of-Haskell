...
# #+LATEX_HEADER: \makeindex[intoc]

# #+LATEX_HEADER: \let\oldindex\index
# #+LATEX_HEADER: \renewcommand{\index}[1]{\oldindex{#1}\marginpar{\strut\footnotesize\color{blue}#1}}
:END:
...
At first the committee had to clearly set the goals of the language and its purposes. January 9th through 12th of 1988 marked the first physical gathering in which the following goals were established [cite:@hudakHistoryHaskellBeing2007]:

1. The language should be appropriate for teaching, doing research, and lastly implementations, such as constructing big systems.

2. Formal syntax and semantics should be presented in order to provide the complete description of the language.
