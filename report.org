...
Lazy evaluation is an evaluation approach that postpones the computation of a statement until the value of it is wanted , so this has as a result the following : the evaluation doesn't spend more time for excessive , and useless calculations. This feature definitely caught the attention of many experts . Many scientists , gathered in order to contribute for the design of the language.
...
Operational semantics in Haskell define how a program is examined, and lazy evaluation is a key aspect of it. Conversely, denotational semantics includes semantics that aren't strict, which describe what a program calculates. The non-strict semantics approach enables the handling of values that are not defined, and also permits the processing of unending data.
